<!-- Product Card -->
<div class="group relative bg-white rounded-2xl overflow-hidden shadow-xl transition-all duration-300 border-2 border-gray-200 {{ $historical ?? false ? 'opacity-60 grayscale' : 'hover:shadow-2xl transform hover:-translate-y-2 hover:border-primary-300' }}">
  <!-- Image Container -->
  @if($historical ?? false)
  ...
  @endif
    @if($product->image)
    <img src="{{ asset($product->image) }}" loading="lazy" alt="{{ $product->name }}" class="h-full w-full object-cover object-center transition duration-500 {{ $historical ?? false ? '' : 'group-hover:scale-110' }}" />
    @if(!($historical ?? false))
