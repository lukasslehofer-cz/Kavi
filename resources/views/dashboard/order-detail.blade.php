...
<div class="space-y-6">
    ...
    <div class="bg-red-50 border border-red-200 rounded-2xl p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                <h3 class="text-base font-bold text-red-800">{{ __('dashboard.payment_not_completed') }}</h3>
                <p class="text-sm text-red-600 mt-1 font-light">{{ __('dashboard.payment_not_completed_description') }}</p>
                <form method="POST" action="{{ localizedRoute('order.pay', $order) }}" class="mt-4">
                    @csrf
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-medium px-6 py-2.5 rounded-full transition-colors text-sm">
                        {{ __('dashboard.pay') }}
                    </button>
                </form>
            </div>
