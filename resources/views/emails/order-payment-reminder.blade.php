...
<head>
    ...
    <title>{{ __('emails.order_payment_reminder.title', [], $locale) }}</title>
    <style>
        /* Reset styles */
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" class="email-container" width="100%" style="width: 100%; max-width: 600px; background-color: #e5e6df !important;" bgcolor="#e5e6df">
                    ...
                    <tr>
                        <td style="padding: 48px 40px; color: #4a4a4a; background-color: #e5e6df;">
                            ...
                            <!-- Message -->
                            <div style="margin: 32px 0; padding: 20px 24px; border-left: 3px solid #D4A053; background-color: #d5d7ca;">
                                <div style="font-size: 11px; font-weight: 400; color: #1c1c1c; text-transform: uppercase; letter-spacing: 0.15em; margin-bottom: 12px;">
                                    {{ $locale === 'cs' ? 'Nedokončená platba' : 'Incomplete payment' }}
                                </div>
                                <p style="font-size: 14px; color: #5a5a5a; margin: 0; line-height: 1.8;">
                                    @if($locale === 'cs')
                                    Všimli jsme si, že platba za vaši objednávku nebyla dokončena. Pokud jste se jen nestihli dostat k platbě, nic se neděje.<br><br>
                                    <span style="color: #D4A053;">&#8594;</span> Klikněte na tlačítko níže a dokončete platbu<br>
                                    <span style="color: #D4A053;">&#8594;</span> <strong style="color: #1c1c1c;">Objednávka bude automaticky zrušena do 24 hodin</strong> od jejího vytvoření, pokud nebude zaplacena
                                    @else
                                    We noticed that the payment for your order was not completed. If you just didn't get around to it, no worries.<br><br>
                                    <span style="color: #D4A053;">&#8594;</span> Click the button below to complete your payment<br>
                                    <span style="color: #D4A053;">&#8594;</span> <strong style="color: #1c1c1c;">Your order will be automatically cancelled within 24 hours</strong> of creation if left unpaid
                                    @endif
                                </p>
                            </div>
                            ...
                            <div style="text-align: center; margin: 40px 0;">
                                <a href="{{ route('dashboard.orders') }}" style="display: inline-block; background-color: #1c1c1c; color: #ffffff !important; text-decoration: none; padding: 16px 32px; font-weight: 400; font-size: 12px; text-transform: uppercase; letter-spacing: 0.15em; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">
                                    {{ __('emails.order_payment_reminder.complete_payment', [], $locale) }} &#8594;
                                </a>
                            </div>
                            ...
                            <div style="margin: 32px 0; padding-top: 24px; border-top: 2px solid #1c1c1c;">
                                ...
                                <p style="font-size: 14px; color: #5a5a5a; line-height: 1.6; margin: 0;">
                                    ...
                                    <span style="color: #D4A053;">&#8594;</span> <strong>{{ __('emails.welcome.email', [], $locale) }}:</strong>
                                    <a href="mailto:{{ $contactEmail }}" style="color: #D4A053; text-decoration: none;">{{ $contactEmail }}</a>
                                </p>
                            </div>
                    ...
                    <tr>
                        <td style="background-color: #d5d7ca; padding: 40px; text-align: center; color: #5a5a5a; font-size: 12px;">
                            ...
                            <div style="margin: 20px 0;">
                                ...
                                <a href="{{ route('dashboard.orders') }}" style="color: #1c1c1c; text-decoration: none; margin: 0 12px; font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em;">{{ $locale === 'cs' ? 'Objednávky' : 'Orders' }}</a>
                            </div>
