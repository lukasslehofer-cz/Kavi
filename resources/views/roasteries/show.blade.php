  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <nav class="text-sm">
      <ol class="flex items-center space-x-2 text-gray-600">
        ...
        <li><a href="{{ route('roasteries.index') }}" class="hover:text-primary-600 transition-colors font-medium">Naše pražírny</a></li>
        <li class="text-gray-400">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
          </svg>
        </li>
        <li class="text-gray-900 font-bold truncate max-w-xs">{{ $roastery->name }}</li>
      </ol>
    </nav>
  </div>
</div>
        ...
        <div>
            ...
            <div class="mb-6">
              <p class="text-lg text-gray-600 font-medium flex items-center gap-2">
                ...
                <span class="font-bold">{{ $roastery->country }}</span>
                @if($roastery->city)
              ...
              @if($roastery->address)
              <p class="text-sm text-gray-500 ml-7 mt-1">{{ $roastery->address }}</p>
              @endif
            </div>
            
            @if($roastery->short_description)
            <p class="text-xl text-gray-600 mb-8 leading-relaxed">{{ $roastery->short_description }}</p>
            @endif
            
            <!-- Links -->
            <div class="flex flex-wrap items-center gap-4 mb-8">
              @if($roastery->website_url)
              <a href="{{ $roastery->website_url }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 bg-gray-900 hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
              ...
              <a href="https://instagram.com/{{ str_replace('@', '', $roastery->instagram) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
        ...
        <div class="lg:sticky lg:top-24 h-fit">
            <div class="relative aspect-square rounded-3xl overflow-hidden shadow-2xl border-2 border-gray-200 bg-gradient-to-br from-gray-100 to-gray-50">
                ...
                <div class="w-full h-full flex flex-col items-center justify-center p-12 bg-gradient-to-br from-primary-100 to-pink-100">
                    ...
                    <p class="text-center text-primary-700 font-semibold">{{ $roastery->name }}</p>
                </div>
                @endif
            </div>
        </div>
    ...
    <div>
      <div class="text-center mb-12">
        <h2 class="text-4xl md:text-5xl font-black text-gray-900 mb-4">
          Naše kávy od <span class="bg-gradient-to-r from-primary-600 to-pink-600 bg-clip-text text-transparent">{{ $roastery->name }}</span>
        </h2>
      ...
      <div class="mb-16">
        ...
        <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 mb-8">
          ...
          <div class="bg-white rounded-2xl shadow-xl overflow-hidden border-2 border-primary-200 hover:shadow-2xl transition-all duration-300">
            <!-- Coffee Image -->
            <div class="relative h-80 overflow-hidden cursor-pointer" onclick="openCoffeeModal({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ $product->image ? asset($product->image) : '' }}', '{{ addslashes($product->short_description ?? '') }}', {{ json_encode($product->attributes ?? []) }})">
              @if($product->image)
        ...
        <div class="bg-gradient-to-r from-primary-600 to-pink-600 rounded-3xl shadow-2xl p-8 md:p-12 text-white">
          <div class="max-w-4xl mx-auto text-center">
            ...
            <h3 class="text-3xl md:text-4xl font-black mb-4">{{ $coffeeOfMonthProducts->count() === 1 ? 'Chcete tuto kávu vyzkoušet?' : 'Chcete tyto kávy vyzkoušet?' }}</h3>
            ...
            <a href="{{ route('subscriptions.index') }}" class="inline-flex items-center gap-2 bg-white text-primary-600 hover:bg-gray-100 font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1">
      ...
      <div>
        ...
        <div class="flex items-center gap-3 mb-6">
          ...
          <h3 class="text-2xl font-black text-gray-900">Další kávy od {{ $roastery->name }}</h3>
        </div>
        ...
        <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border-2 border-gray-200">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              ...
              <a href="{{ route('subscriptions.index') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-primary-600 to-pink-600 hover:from-primary-700 hover:to-pink-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                <span>Vybrat předplatné</span>
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
