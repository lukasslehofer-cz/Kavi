...
@section('title', $locale === 'en' ? 'Page Expired | KAVI' : 'Platnost stránky vypršela | KAVI.cz')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-2">
            ...
            <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold uppercase tracking-tight mb-6 leading-[0.95]">
                {{ $locale === 'en' ? 'Page' : 'Platnost' }}<br>
                <span class="text-red-600">{{ $locale === 'en' ? 'Expired' : 'Vypršela' }}</span>
            </h1>
            ...
            <div class="flex flex-wrap gap-4 mb-12">
                <button onclick="window.location.reload()" class="inline-flex items-center justify-center bg-[#1c1c1c] text-white text-xs uppercase tracking-widest px-8 py-4 hover:bg-[#1c1c1c]/80 transition-colors">
                    {{ $locale === 'en' ? 'Refresh Page' : 'Obnovit stránku' }}
                </button>
            ...
            <div class="pt-8 border-t border-[#1c1c1c]/20">
                <p class="text-xs uppercase tracking-widest text-[#1c1c1c]/50 mb-4">
                    {{ $locale === 'en' ? 'Why did this happen?' : 'Proč se to stalo?' }}
                </p>
        ...
        <div class="lg:col-span-1">
            <div class="bg-[#BCBEB1] p-8 h-full">
                <h2 class="text-xs uppercase tracking-widest mb-6 border-b border-[#1c1c1c] pb-4">
                    {{ $locale === 'en' ? 'Quick Fix' : 'Rychlé řešení' }}
                </h2>
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm">
                                {{ $locale === 'en' ? 'Click the "Refresh Page" button above' : 'Klikněte na tlačítko "Obnovit stránku" výše' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm">
                                {{ $locale === 'en' ? 'Or press Ctrl+R (Cmd+R on Mac)' : 'Nebo stiskněte Ctrl+R (Cmd+R na Macu)' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm">
                                {{ $locale === 'en' ? 'Fill out the form again' : 'Vyplňte formulář znovu' }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-6 border-t border-[#1c1c1c]/20">
                    ...
                    <p class="text-sm mt-1">419 / {{ $locale === 'en' ? 'Session Expired' : 'Relace vypršela' }}</p>
                </div>
            </div>
        </div>
    </div>
