            ...
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                try {
                    ...
                    if ($result['success']) {
                        ...
                        // Handle payment failure OUTSIDE transaction so DB changes persist
                        if ($result['requires_failure_handling'] ?? false) {
                            $stripeService->handleSubscriptionPaymentFailure($subscription, $result['error']);
                        }
                    ...
                    // Handle payment failure OUTSIDE transaction so DB changes persist
                    $stripeService->handleSubscriptionPaymentFailure($subscription, $e->getMessage());
