...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Výchozí režim (vlastní pořadí) umožňuje přetahování - proto bez stránkování
        $sort = $request->get('sort', 'default');
        $reorderMode = $sort === 'default';
        ...
        return view('admin.products.index', compact('products', 'stats', 'reorderMode'));
    }

    /**
     * Uložit nové pořadí produktů (drag & drop ve výpisu).
     *
     * Přijímá pouze ID viditelných a přetahovatelných produktů. Aby přetahování
     * fungovalo i se zapnutým filtrem kategorie (a s neaktivními produkty, které
     * se netáhnou), přeuspořádá se tažená množina jen mezi pozicemi, které už
     * v celkovém pořadí zabírala - ostatní produkty si své místo podrží.
     */
    public function reorder(Request $request)
    {
        ...
        DB::transaction(function () use ($ids) {
            // Kanonické současné pořadí VŠECH produktů (shodné s veřejným webem)
            $current = Product::orderBy('sort_order')
                ->orderBy('created_at', 'desc')
                ->pluck('sort_order', 'id');

            $order = array_map('intval', $current->keys()->all());
            $dragged = array_flip($ids);

            // Pozice, které tažená množina zabírá v kanonickém pořadí
            $slots = [];
            ...
            // Do těchto pozic nasypat nové pořadí
            foreach ($slots as $key => $slotIndex) {
                $order[$slotIndex] = $ids[$key];
            }

            // Zapsat husté pořadí 1..N, ale jen tam, kde se hodnota opravdu mění
            foreach ($order as $index => $id) {
                if ((int) $current[$id] !== $index + 1) {
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($validated['altitude'])) {
            $validated['altitude'] = $this->stripAltitudeUnit($validated['altitude']);
        }

        // Bez zadaného pořadí zařadit produkt na konec (jinak by skočil na začátek e-shopu)
        if (!$request->filled('sort_order')) {
            $validated['sort_order'] = (int) Product::max('sort_order') + 1;
        }
    ...
    private function stripAltitudeUnit(?string $value): ?string
    {
        if (empty($value)) {
            return $value;
        }
